...
/** \brief All C++ FastDeploy benchmark profile APIs are defined inside this
 * namespace
 *
 */
namespace benchmark {
...
struct BenchmarkOption {
  ...
  friend std::ostream &operator<<(std::ostream &output,
    ...
    if (!option.include_h2d_d2h) {
      output << "Running profiling for Runtime " << "without H2D and D2H, ";
    } else {
      output << "Running profiling for Runtime " << "with H2D and D2H, ";
    }
