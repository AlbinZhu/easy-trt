...
// model::BaseModel::BaseModel(const utils::InitParameter &param) :
// m_param(param) {
//   // input
...
//       param.batch_size * (1 + param.topK * m_output_objects_width); // 1:
//       count
...
bool model::BaseModel::init(const std::vector<unsigned char> &trtFile) {
  if (trtFile.empty()) {
    return false;
  }
  ...
  if (dynamic) // for some models only support static mutil-batch.
               // eg: yolox
  {
    this->m_context->setInputShape(
        this->m_engine->getIOTensorName(0),
        nvinfer1::Dims4(batch_size, 3, dst_h, dst_w));
  }
  m_output_dims =
  ...
  CHECK(cudaMalloc(&m_output_src_device,
                   batch_size * m_output_area * sizeof(float)));
  float a = float(dst_h) / src_h;
  float b = float(dst_w) / src_w;
  float scale = a < b ? a : b;
  cv::Mat src2dst = (cv::Mat_<float>(2, 3) << scale, 0.f,
                     (-scale * src_w + dst_w + scale - 1) * 0.5, 0.f, scale,
                     (-scale * src_h + dst_h + scale - 1) * 0.5);
...
void model::BaseModel::check() {
  nvinfer1::Dims dims;

  sample::gLogInfo << "the engine's info:" << std::endl;
  std::cout << "input: " << input_output_names[0];
  for (auto layer_name : input_output_names) {
...
void model::BaseModel::copy(const std::vector<cv::Mat> &imgsBatch) {
#if 0 
  ...
  for (size_t i = 0; i < imgsBatch.size(); i++) {
    CHECK(cudaMemcpy(pi, imgsBatch[i].data,
                     sizeof(unsigned char) * 3 * src_h * src_w,
...
void model::BaseModel::preprocess(const std::vector<cv::Mat> &imgsBatch) {
  resizeDevice(batch_size, m_input_src_device, src_w, src_h,
               m_input_resize_device, dst_w, dst_h, 114, m_dst2src);
  bgr2rgbDevice(batch_size, m_input_resize_device, dst_w, dst_h,
                m_input_rgb_device, dst_w, dst_h);
  normDevice(batch_size, m_input_rgb_device, dst_w, dst_h, m_input_norm_device,
             dst_w, dst_h, scale, means, stds);
  hwc2chwDevice(batch_size, m_input_norm_device, dst_w, dst_h,
                m_input_hwc_device, dst_w, dst_h);
}

bool model::BaseModel::infer() {
  float *bindings[] = {m_input_hwc_device, m_output_src_device};
  bool context = m_context->executeV2((void **)bindings);
  return context;
}

// void model::BaseModel::postprocess(const std::vector<cv::Mat> &imgsBatch) {
...
//                    m_param.batch_size * sizeof(float) * (1 + 7 *
//                    m_param.topK), cudaMemcpyDeviceToHost));
...
// std::vector<std::vector<utils::Box>> model::BaseModel::getObjectss() const {
//   return this->m_objectss;
// }

// void model::BaseModel::reset() {
//   CHECK(
//       cudaMemset(m_output_objects_device, 0,
//                  sizeof(float) * m_param.batch_size * (1 + 7 *
//                  m_param.topK)));
