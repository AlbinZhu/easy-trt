...
model::DetModel::DetModel(const utils::InitParameter &param) : m_param(param) {

  this->src_w = param.src_w;
  this->src_h = param.src_h;
  this->dst_w = param.dst_w;
  this->dst_h = param.dst_h;
  this->scale = param.scale;
  this->means = param.means;
  this->stds = param.stds;
  this->input_output_names = param.input_output_names;
  std::cout << "------------init------------" << std::endl;
  // input
  std::cout << "src: " << param.input_output_names[0] << std::endl;
  ...
  std::cout << "src_w: " << param.src_w << std::endl;
...
// bool model::DetModel::init(const std::vector<unsigned char> &trtFile) {
//   if (trtFile.empty()) {
//     return false;
//   }
...
//     return false;
//   }
//   if (m_param.dynamic_batch) // for some models only support static
//   mutil-batch.
//                              // eg: yolox
//   {
//     this->m_context->setInputShape(
//         this->m_engine->getIOTensorName(0),
//         nvinfer1::Dims4(m_param.batch_size, 3, m_param.dst_h,
//         m_param.dst_w));
//   }
//   m_output_dims =
...
//        (-scale * m_param.src_w + m_param.dst_w + scale - 1) * 0.5, 0.f,
//        scale,
...
//   }
// }
// void model::DetModel::copy(const std::vector<cv::Mat> &imgsBatch) {
// #if 0
//     cv::Mat img_fp32 = cv::Mat::zeros(imgsBatch[0].size(), CV_32FC3); // todo
//     cudaHostRegister(img_fp32.data, img_fp32.elemSize() * img_fp32.total(),
//     cudaHostRegisterPortable); float* pi = m_input_src_device; for (size_t i
//     = 0; i < imgsBatch.size(); i++)
//     {
//         imgsBatch[i].convertTo(img_fp32, CV_32FC3);
//         CHECK(cudaMemcpy(pi, img_fp32.data, sizeof(float) * 3 * m_param.src_h
//         * m_param.src_w, cudaMemcpyHostToDevice)); pi += 3 * m_param.src_h *
//         m_param.src_w;
//     }
...
//         std::vector<float>(imgsBatch[i].reshape(1, 1));
//         imgsBatch[i].convertTo(img_fp32, CV_32FC3);
//         CHECK(cudaMemcpy(pi, img_fp32.data, sizeof(float) * 3 * m_param.src_h
//         * m_param.src_w, cudaMemcpyHostToDevice)); pi += 3 * m_param.src_h *
//         m_param.src_w;
...
//                      m_param.src_w, cudaMemcpyHostToDevice));
...
//     for (size_t i = 0; i < imgsBatch.size(); i++)
//     {
//         CHECK(cudaMemcpyAsync(pi, imgsBatch[i].data, sizeof(unsigned char) *
//         3 * m_param.src_h * m_param.src_w, cudaMemcpyHostToDevice,
//         streams[i])); pi += 3 * m_param.src_h * m_param.src_w;
//     }
//     CHECK(cudaDeviceSynchronize());
// #endif
// }

// void model::DetModel::preprocess(const std::vector<cv::Mat> &imgsBatch) {
...
//              m_param.dst_h, m_input_norm_device, m_param.dst_w,
  ...
  for (size_t bi = 0; bi < imgsBatch.size(); bi++) {
    int num_boxes =
        std::min((int)(m_output_objects_host +
                 ...
                 m_param.topK);
    std::cout << "boxes: " << num_boxes << std::endl;
...
std::vector<std::vector<utils::Box>> model::DetModel::getObjectss() const {
  return this->m_objectss;
}

void model::DetModel::reset() {
  CHECK(
