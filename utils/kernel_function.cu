...
__device__ void affine_project_device_kernel(utils::AffineMat *matrix, int x,
                                             int y, float *proj_x,
                                             float *proj_y) {
...
__global__ void resize_rgb_padding_device_kernel(
    float *src, int src_width, int src_height, int src_area, int src_volume,
...
__global__ void
resize_rgb_padding_device_kernel(unsigned char *src, int src_width,
                                 int src_height, int src_area, int src_volume,
                                 float *dst, int dst_width, int dst_height,
                                 int dst_area, int dst_volume, int batch_size,
  ...
  if (dx < dst_area && dy < batch_size) {
    ...
    if (src_x < -1 || src_x >= src_width || src_y < -1 || src_y >= src_height) {
      ...
      int y_high = y_low + 1;
      int x_high = x_low + 1;
      unsigned char const_values[] = {(unsigned char)padding_value,
...
__global__ void resize_rgb_without_padding_device_kernel(
    float *src, int src_width, int src_height, int src_area, int src_volume,
    float *dst, int dst_width, int dst_height, int dst_area, int dst_volume,
...
__global__ void resize_gray_without_padding_device_kernel(
    float *src, int src_width, int src_height, int src_area, float *dst,
    int dst_width, int dst_height, int dst_area, int batch_size,
...
__global__ void bgr2rgb_device_kernel(float *src, float *dst, int batch_size,
...
__global__ void norm_device_kernel(float *src, float *dst, int batch_size,
                                   int img_height, int img_width, int img_area,
                                   int img_volume, float scale, float mean0,
                                   float mean1, float mean2, float std0,
  ...
  if (dx < img_volume && dy < batch_size) {
    ...
    switch (ch) {
    case 0:
      dst[dy * img_volume + dx] =
          norm_device(src[dy * img_volume + dx], scale, mean0, std0);
      break;
    case 1:
      dst[dy * img_volume + dx] =
          norm_device(src[dy * img_volume + dx], scale, mean1, std1);
      break;
    case 2:
      dst[dy * img_volume + dx] =
          norm_device(src[dy * img_volume + dx], scale, mean2, std2);
      break;
    }
  }
}

__global__ void hwc2chw_device_kernel(float *src, float *dst, int batch_size,
                                      int img_height, int img_width,
                                      int img_area, int img_volume) {
  int dx = blockDim.x * blockIdx.x + threadIdx.x;
  int dy = blockDim.y * blockIdx.y + threadIdx.y;
...
void resizeDevice(const int &batchSize, float *src, int srcWidth, int srcHeight,
  ...
  resize_rgb_padding_device_kernel<<<grid_size, block_size, 0, nullptr>>>(
      src, srcWidth, srcHeight, src_area, src_volume, dst, dstWidth, dstHeight,
      dst_area, dst_volume, batchSize, paddingValue, matrix);
}
...
void resizeDevice(const int &batchSize, float *src, int srcWidth, int srcHeight,
  ...
  case utils::ColorMode::RGB:
    resize_rgb_without_padding_device_kernel<<<grid_size, block_size, 0,
                                               nullptr>>>(
        src, srcWidth, srcHeight, src_area, src_volume, dst, dstWidth,
        dstHeight, dst_area, dst_volume, batchSize, matrix);
    return;
  case utils::ColorMode::GRAY:
    resize_gray_without_padding_device_kernel<<<grid_size, block_size, 0,
                                                nullptr>>>(
        src, srcWidth, srcHeight, src_area, dst, dstWidth, dstHeight, dst_area,
        batchSize, matrix);
    return;
  }
}

void bgr2rgbDevice(const int &batchSize, float *src, int srcWidth,
                   int srcHeight, float *dst, int dstWidth, int dstHeight) {
  dim3 block_size(BLOCK_SIZE, BLOCK_SIZE);
...
__global__ void decode_yolo_device_kernel(int batch_size, int num_class,
...
__global__ void get_key_val_kernel(int batchSize, float *src, int srcWidth,
                                   int srcHeight, int srcArea, int *idx,
                                   float *conf) {
  int dx = blockDim.x * blockIdx.x + threadIdx.x;
  int dy = blockDim.y * blockIdx.y + threadIdx.y;
...
__global__ void copy_with_padding_kernel_function(
    int batchSize, float *src, int srcWidth, int srcHeight, int srcArea,
    int srcVolume, float *dst, int dstWidth, int dstHeight, int dstArea,
    int dstVolume, float paddingValue, int padTop, int padLeft) {
  int dx = blockDim.x * blockIdx.x + threadIdx.x;
  int dy = blockDim.y * blockIdx.y + threadIdx.y;
...
void nmsDeviceV1(utils::InitParameter param, float *src, int srcWidth,
                 int srcHeight, int srcArea) {
  dim3 block_size(BLOCK_SIZE, BLOCK_SIZE);
  ...
  nms_fast_kernel<<<grid_size, block_size, 0, nullptr>>>(
      param.topK, param.batch_size, param.iou_thresh, src, srcWidth, srcHeight,
      srcArea);
}

void nmsDeviceV2(utils::InitParameter param, float *src, int srcWidth,
                 int srcHeight, int srcArea, int *idx, float *conf) {
  dim3 block_size(BLOCK_SIZE, BLOCK_SIZE);
  dim3 grid_size((param.topK + BLOCK_SIZE - 1) / BLOCK_SIZE,
                 (param.batch_size + BLOCK_SIZE - 1) / BLOCK_SIZE);
  get_key_val_kernel<<<grid_size, block_size, 0, nullptr>>>(
      param.batch_size, src, srcWidth, srcHeight, srcArea, idx, conf);
  for (size_t i = 0; i < param.batch_size; i++) {
    int *p_idx = idx + i * srcHeight;
    float *p_conf = conf + i * srcHeight;
    thrust::sort_by_key(thrust::device, p_conf, p_conf + srcHeight, p_idx,
...
void copyWithPaddingDevice(const int &batchSize, float *src, int srcWidth,
                           int srcHeight, float *dst, int dstWidth,
                           int dstHeight, float paddingValue, int padTop,
                           int padLeft) {
  dim3 block_size(BLOCK_SIZE, BLOCK_SIZE);
  dim3 grid_size((dstWidth * dstHeight + BLOCK_SIZE - 1) / BLOCK_SIZE,
                 (batchSize + BLOCK_SIZE - 1) / BLOCK_SIZE);
  ...
  copy_with_padding_kernel_function<<<grid_size, block_size, 0, nullptr>>>(
      batchSize, src, srcWidth, srcHeight, src_area, src_volume, dst, dstWidth,
      dstHeight, dst_area, dst_volume, paddingValue, padTop, padLeft);
}
