...
std::string utils::getSystemTimeStr() { return std::to_string(std::rand()); }

bool utils::setInputStream(const utils::InputStream &source,
  ...
  case utils::InputStream::IMAGE:
    ...
    capture.open(imagePath); // cv::CAP_IMAGES : !< OpenCV Image Sequence (e.g.
                             // img_%02d.jpg)
    param.batch_size = 1;
    total_frames = 1;
    totalBatches = 1;
...
void utils::show(const std::vector<std::vector<utils::Box>> &objectss,
  ...
  if (!imgsBatch[0].empty()) {
    cv::namedWindow(windows_title,
                    cv::WINDOW_NORMAL |
                        cv::WINDOW_KEEPRATIO); // allow window resize(Linux)

    int max_w = 960;
    int max_h = 540;
    if (imgsBatch[0].rows > max_h || imgsBatch[0].cols > max_w) {
      cv::resizeWindow(windows_title, max_w,
                       imgsBatch[0].rows * max_w / imgsBatch[0].cols);
    }
  }
      ...
      for (auto &box : objectss[bi]) {
        ...
        if (!box.land_marks.empty()) // for facial landmarks
        {
          for (auto &pt : box.land_marks) {
            cv::circle(imgsBatch[bi], pt, 1, cv::Scalar(255, 255, 255), 1,
                       cv::LINE_AA, 0);
          }
        }
      }
...
void utils::save(const std::vector<std::vector<Box>> &objectss,
                 const std::vector<std::string> &classNames,
...
utils::HostTimer::HostTimer() { t1 = std::chrono::steady_clock::now(); }
